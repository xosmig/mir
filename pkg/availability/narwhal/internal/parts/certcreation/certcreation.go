...
import (
	...
	"github.com/filecoin-project/mir/pkg/availability/narwhal/internal/common"
	mscdsl "github.com/filecoin-project/mir/pkg/availability/narwhal/internal/dsl"
	"github.com/filecoin-project/mir/pkg/availability/narwhal/internal/protobuf"
	batchstoragedsl "github.com/filecoin-project/mir/pkg/batchstorage/dsl"
...
func IncludeCreatingCertificates(
	...
	adsl.UponRequestCert(m, func(origin *apb.RequestCertOrigin) error {
		batchstoragedsl.StoreBatchOnQuorum(m, mc.BatchStorage, TODO, TODO)
		return nil
	})

	batchstoragedsl.UponBatchStoredOnQuorum(m, func(context *TODO) error {
	...
	mempooldsl.UponNewBatch(m, func(txIDs []t.TxID, txs []*requestpb.Request, context *requestBatchFromMempoolContext) error {
		...
		mempooldsl.RequestBatchID(m, mc.BatchStorage, txIDs, &requestIDOfOwnBatchContext{context.reqID, txs})
		return nil
	})
	...
	mscdsl.UponRequestSigMessageReceived(m, func(from t.NodeID, txs []*requestpb.Request, reqID RequestID) error {
		mempooldsl.RequestTransactionIDs(m, mc.BatchStorage, txs, &computeIDsOfReceivedTxsContext{from, txs, reqID})
		return nil
	})
	...
	mempooldsl.UponTransactionIDsResponse(m, func(txIDs []t.TxID, context *computeIDsOfReceivedTxsContext) error {
		...
		mempooldsl.RequestBatchID(m, mc.BatchStorage, txIDs,
