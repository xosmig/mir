...
import (
	...
	mpevents "github.com/filecoin-project/mir/pkg/mempool/mempoolevents"
...
func NewBatch(m dsl.Module, dest t.ModuleID, txs [][]byte, origin *mppb.RequestBatchOrigin) {
	dsl.EmitEvent(m, mpevents.NewBatch(dest, txs, origin))
}
...
func UponNewBatch[C any](m dsl.Module, handler func(txs [][]byte, context *C) error) {
